...
function setMenus(const items : array of string) : TArrayOfString;
var
  i, l : integer;
begin
  l := high(items);
  setLength(result, l + 1);
  for i := 0 to l do result[i] := items[i]
end;

procedure setColor(const fore, back : byte);
begin
  textColor(fore);
  textBackground(back);
end;

procedure printMenu(const items : TArrayOfString; const selected : integer);
var
  i : integer;
begin
...
function selectMenu(const items : TArrayOfString; const top : integer) : integer;
var
  ch : char;
  i, l : integer;
begin
