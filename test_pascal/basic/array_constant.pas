...
{n - счетчик тел змейки}
    snake: array [1.. 2, 1.. lm]of integer;

{массив координат тел змейки}
    appl: boolean;

{наличие яблока на поле}
...
{сканирование первого тела на собственный цвет}
var
    t: integer;
...
begin
    for t := - k + 2 to k - 2 do
    begin
        if getpixel(x + t, y) = snake_color  then
            ending;
        if getpixel(x, y + t) = snake_color  then
            ending;
    end;
    if (x <= 10 + k) or (x >= 630 - k) or
            {если голова вышла за пределы поля}
    (y <= 40 + k) or (y >= 470 - k) then
        ending;
end;
procedure point;

{изменение кол-ва очков и вывод на экран}
begin
    inc(points, 5 * sp);
    sp := (points div 100) + 1;
    str(points, spoints);
...
begin
    ...
    begin
        ...
        if j <= n  then
            krug(snake[1, lm - n + j], snake[2, lm - n + j], 0)
                {удаление последнего тела}

        else
            krug(snake[1, j - n], snake[2, j - n], 0);

        {с целью имитации движения}
...
begin
    ...
    {переменная направления (в часах поворота)}
    n := 2;
