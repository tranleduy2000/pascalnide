...
var     p: longint;   tg: int64;
begin
        while c div 2>0 do
        begin
                p:=c div 2;
                if d[h[p]]<=d[h[c]] then exit;
                tg:=pos[h[p]]; pos[h[p]]:=pos[h[c]];  pos[h[c]]:=tg;
                tg:=h[p]; h[p]:=h[c];  h[c]:=tg;
                c:=p;
        end;
end;

procedure downheap(p: longint);
var     c: longint;  tg: int64;
begin
        while p*2<=nheap do
        begin
                c:=p*2;
...
var     i,j,u,v,cx: longint;  tg: contro;
begin
        for i:=1 to n do
        begin
                d[i]:=oo;  pos[i]:=0;
...
var     i,j,u,v,cx,cb: longint;  tg: contro;
begin
        for i:=1 to n do
        begin
                d[i]:=oo;  pos[i]:=0;
...
procedure doc;
var     i,u,v,cb,cx: longint;  res,tmp: int64;
begin
