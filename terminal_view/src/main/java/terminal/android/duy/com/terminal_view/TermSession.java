...
public class TermSession {
    public static final String TAG = TermSession.class.getSimpleName();
    ...
    public TermSession(final boolean exitOnEOF) {
        ...
        mWriterThread = new Thread() {
                ...
                mWriterHandler = new Handler() {
                    ...
                    public void handleMessage(Message msg) {
                        if (msg.what == NEW_OUTPUT) {
                            Log.d(TAG, "handleMessage: write output");
                            writeToOutput();
            ...
            private void writeToOutput() {
                Log.d(TAG, "mWriterThread writeToOutput: ");
    ...
    private void notifyNewOutput() {
        Log.d(TAG, "notifyNewOutput: ");
        Handler writerHandler = mWriterHandler;
        if (writerHandler == null) {
           /* Writer thread isn't started -- will pick up data once it does */
            Log.d(TAG, "notifyNewOutput: Writer thread isn't started -- will pick up data once it does");
            return;
        }
