...
//                Log.v("SpartacusRex","mapControlChar RELEASE CTRL from META");
            ...
            if (result > '0' && result <= '9') {
                int offs = (int) '1';
                result = KEYCODE_OFFSET + TermKeyFileListener.KEYCODE_F1 + result - offs;
            } else if (result == '0') {
                result = KEYCODE_OFFSET + TermKeyFileListener.KEYCODE_F10;
            } else if (result == 'o' || result == 'O') {
                result = KEYCODE_OFFSET + TermKeyFileListener.KEYCODE_F11;
            } else if (result == 'p' || result == 'P') {
                result = KEYCODE_OFFSET + TermKeyFileListener.KEYCODE_F12;
            }
...
//        Log.v("SpartacusRex","mapControlChar "+ch+" --> "+result);
    ...
    public void keyDown(int keyCode, KeyEvent event, EditText out, boolean appMode) throws IOException {
...
//            Log.v("SpartacusRex","EMVIEW : FORW DELETE");
                result = KEYCODE_OFFSET + TermKeyFileListener.KEYCODE_FORWARD_DEL;
                break;
...
//                   (mAltKey.isActive() ? KeyEvent.META_ALT_ON : 0));
        ...
        if (result >= KEYCODE_OFFSET) {
            ...
            if (mAltKey.isActive()) {
                //Send Escape First..
                out.getEditableText().insert(out.getSelectionStart(), Character.toString((char) 27));
            }

//            Log.v("SpartacusRex","TERM : write code:"+keyCode+" result:"+result);
            out.getEditableText().insert(out.getSelectionStart(), Character.toString((char) result));
        }
    }

    public boolean handleKeyCode(int keyCode, EditText out, boolean appMode) throws IOException {
        ...
        if (keyCode >= -22 && keyCode <= -19) {
            ...
            for (int i = 0; i < length; i++) {
                ...
                out.getEditableText().insert(out.getSelectionStart(), Character.toString(cc));
            }
