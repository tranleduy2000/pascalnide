...
public enum OperatorTypes {
    ...
    MULTIPLY(false, false) {
        @Override
        public boolean verifyBinaryOperation(Type GCF) {
    ...
    public static Type get_GCF(Type one, Type two) {
    ...
    public boolean verifyBinaryOperation(Type type1, Type type2)
            throws BadOperationTypeException {
        Type GCF = get_GCF(type1, type2);
        return GCF != null && verifyBinaryOperation(GCF);
    }

    boolean verifyBinaryOperation(Type GCF) {
        return true;
    }
